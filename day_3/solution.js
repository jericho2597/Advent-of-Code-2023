const input = `.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
...
const processPartOne = (input) => {
    ...
    // store the nums as objects of the form mapped from their row num
    //   {
    //      col: ..
    //      length: ..
    //      value: ..
    //   }
    const numsFound = new Map();

    // store the symbols as their col, mapped from their row num
    const symbolsFound = new Map();

    // initialise empty array of nums found and symbols found
    for (let i = 0; i < height; i++) {
        numsFound.set(i, []);
        symbolsFound.set(i, []);
    }
    numsFound.set(-1, []);
    numsFound.set(height, []);

    // parse all num and symbol positions
    for (let row = 0; row < height; row++) {
        const line = lines[row];
        let numstring = '';
        for (let col = 0; col < width; col++) {
            const currentChar = line.charAt(col);

            if (currentChar === '.') {
                // end of a number has been read, so add to map
                if (numstring != '') {
                    numsFound.get(row).push({
                        value: parseInt(numstring),
                        length: numstring.length,
                        col: (col - numstring.length),
                    })
                    numstring = '';
                }
            }
        ...
        if (numstring != '') {
            numsFound.get(row).push({
                value: parseInt(numstring),
                length: numstring.length,
                col: (width - 1 - numstring.length),
            })
            numstring = '';
        }
    }

    let validNums = new Map();

    // loop through each row
    for (let row = 0; row < height; row++) {
        symbolsOnRow = symbolsFound.get(row);
        // each symbol is stored as it's col value
        for (let col of symbolsOnRow) {
            // now we have the row and col of the symbol, look for
            // numbers that could match in the adjacent rows
            // console.log("symbol at: [" + row + ", " + col + "]");
            for (let i = row - 1; i <= row + 1; i++) {
                numbersOnRow = numsFound.get(i);
                for (let num of numbersOnRow) {
                    // we know the symbol is within 1 row of the num
                    // so as long as it's col falls within 1 of the num
                    // then the num is a match.
                    if (col >= num.col - 1 && col <= num.col + num.length) {
                        // console.log("valid num: ");
                        // console.log(num);
                        validNums.set({ row: i, col: num.col }, num.value);
                    }
                }
            }
        }
            ...
            for (let i = row - 1; i <= row + 1; i++) {
                ...
                for (let num of numbersOnRow) {
                    ...
                    if (col >= num.col - 1 && col <= num.col + num.length) {
                        // console.log("valid num: ");
                        // console.log(num);
                        validNums.push(num.value);
                    }
                }
            }
            if (validNums.length == 2) {
                sum += validNums[0] * validNums[1];
            }
        }
    }
