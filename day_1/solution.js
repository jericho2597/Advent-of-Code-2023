...
/**
 * Finds the first and last digit in each line of the input string, combines
 * tham to make a two digit number, and sums all the numbers as the final calibration result.
 * 
 * Any line that only has one digit, this digit is treated as both the first and last digit.
 * 
 * E.g. 
 *          kfxone67bzb2   => 62
 *          8jjpseven      => 88
...
const calibratePartOne = (input) => {
    ...
    for (let line of lines){
        ...
        for(let i = 0; i < length; i ++) {
            let c = line[i];
            if (!isNaN(c)){
                if (firstDigit == -1) { firstDigit = parseInt(c);}
                else { lastDigit = parseInt(c);}
            }
        }

        value = firstDigit * 10;
        // if last digit not found, first digit is also last digit
        if(lastDigit == -1){
            value += firstDigit;
        }
        else {
...
/**
 * In the second part of challenge, numbers in text are also counted as digits
 * 
 * E.g. 
 *          kfxone67bzb2   => 12
 *          8jjpseven      => 87
 *          6eighthree     => 63 (tricky edge case where multiple num strings share chars)
 * 
 * @param {*} input 
 */
const calibratePartTwo = (input) => {
    let processedInput = input.replaceAll("one", "one1one");
    processedInput = processedInput.replaceAll("two","two2two");
    processedInput = processedInput.replaceAll("three","three3three");
    processedInput = processedInput.replaceAll("four","four4four");
    processedInput = processedInput.replaceAll("five","five5five");
    processedInput = processedInput.replaceAll("six","six6six");
    processedInput = processedInput.replaceAll("seven","seven7seven");
    processedInput = processedInput.replaceAll("eight","eight8eight");
    processedInput = processedInput.replaceAll("nine","nine9nine");
...
calibratePartOne(input);
console.log("PART TWO");
calibratePartTwo(input);