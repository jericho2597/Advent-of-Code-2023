const input = `Game 1: 7 blue, 9 red, 1 green; 8 green; 10 green, 5 blue, 3 red; 11 blue, 5 red, 1 green
...
const playPartOne = (input) => {
    ...
    for (let line of lines) {
        ...
        let idAndGame = line.split(": ");
        let id = parseInt(idAndGame[0]);
        line = idAndGame[1];
        let viewings = line.split('; ');
        let invalid = false;
        for (let viewing of viewings) {
            let ballCounts = viewing.split(', ');
            for (let ballCount of ballCounts) {
                let pair = ballCount.split(' ');
                let count = parseInt(pair[0]);
                console.log(pair);

                if (pair[1] == "red" && count > 12) {
                    invalid = true;
                    console.log("Found invalid count of red");
                    break;
                }
                else if (pair[1] == "green" && count > 13) {
                    invalid = true;
                    console.log("Found invalid count of green");
                    break;
                }
                else if (pair[1] == "blue" && count > 14) {
                    invalid = true;
                    console.log("Found invalid count of blue");
                    break;
                }
            }
            if (invalid) { break; }
    ...
    for (let line of lines) {
        ...
        for (let viewing of viewings) {
            ...
            for (let ballCount of ballCounts) {
                ...
                if (pair[1] == "red" && count > minRed) {
                    minRed = count;
                    console.log("Found new min count of red");
                }
                else if (pair[1] == "green" && count > minGreen) {
                    minGreen = count;
                    console.log("Found new min count of green");
                }
                else if (pair[1] == "blue" && count > minBlue) {
                    minBlue = count
                    console.log("Found new min count of blue");
                }
            }
        }
